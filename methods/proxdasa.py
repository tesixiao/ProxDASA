...
class PROXDASA:
    ...
    def __init__(self, local_params, mixing_matrix, training_data, init_weights):
        ...
        if 'comm_round' in local_params:
            ...
            self.comm_round = int(math.log(2 * self.num_nodes + 3) / 2)
        ...
        # ProxDASA: Initialize local y and z
    ...
    def solve(self, outer_iterations, training_data_full_sample, testing_data):
        ...
        for i in range(outer_iterations):
            ...
            if self.step_type == 'constant':
                ...
                self.alpha = min(self.alpha_base * math.sqrt(self.num_nodes / (i+1)), 1)
...
if __name__=='__main__':
    ...
    # Parse user input
    parser = argparse.ArgumentParser(description='Testing PROXDASA on problems from paper.')

    parser.add_argument('--updates', type=int, default=3001, help='Total number of communication rounds.')
    parser.add_argument('--lr', type=float, default=3.0, help='Local learning rate.')
    parser.add_argument('--alpha_base', type=float, default=3.0, help='Moving average rate base')
    ...
    parser.add_argument('--data', type=str, default='mnist', choices=['a9a', 'mnist'], help='Dataset.')
    ...
    if rank == 0:
        opening_statement = f' PROXDASA on {args.data} '
        print(f"\n{'#' * 75}")
    ...
    solver = PROXDASA(algo_params, mixing_matrix, train_loader, init_weights)
