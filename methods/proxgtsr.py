...
"""
Test ProxGT-SR
    [1] https://arxiv.org/pdf/2110.01594.pdf
...
class ProxGTSR:
    ...
    def __init__(self, local_params, mixing_matrix, training_data, init_weights):
        ...
        # full_grad = how often to compute the full gradient (INT)
        ...
        if 'lr' in local_params:
            ...
            self.lr = 1e-2
        if 'full_grad' in local_params:
            self.full_grad = local_params['full_grad']
        else:
            self.full_grad = 32
        ...
        self.grads = [torch.zeros(size=p.shape).to(self.device) for p in self.model.parameters()]
        self.Y = [torch.zeros(size=p.shape).to(self.device) for p in self.model.parameters()]
        self.prev_weights = [torch.zeros(size=p.shape).to(self.device) for p in self.model.parameters()]
        self.V = [torch.zeros(size=p.shape).to(self.device) for p in self.model.parameters()]
        self.prev_V = [torch.zeros(size=p.shape).to(self.device) for p in self.model.parameters()]
    ...
    def solve(self, outer_iterations, training_data_full_sample, testing_data):
        ...
        self.Y = self.get_full_grad(self.weights, training_data_full_sample)
        self.V = [self.Y[pa].detach().clone() for pa in range(len(self.Y))]
        ...
        for i in range(outer_iterations):
            ...
            # GRADIENT ROUTINE
            if i % self.full_grad == 0 and i > 0:

                # Compute a full gradient
                self.V = self.get_full_grad(self.weights, training_data_full_sample)
            else:

                # Compute a sample gradient
                grad_diff = self.get_grads(self.weights, self.prev_weights)
                self.V = [grad_diff[j] + self.prev_V[j] for j in range(len(grad_diff))]
            ...
            # Communicate Y
            comm.Barrier()
            comm_time2 = self.communicate_y_with_neighbors()
            comm.Barrier()
            ...
            # UPDATE WEIGHTS
            self.weights = self.regularizer.forward([self.weights[k] - self.lr * self.Y[k].to(self.device) for k in
                                                     range(self.num_params)], self.lr * self.l1)

            # END TIME
            int_time2_end = time.time()

            # Save pre communication weights for this method
            ...
            # UPDATE Y AND V
            self.Y = [self.Y[j] + self.V[j] - self.prev_V[j] for j in range(len(self.Y))]
            self.prev_V = [self.V[pa].detach().clone() for pa in range(len(self.V))]
            ...
            if i % self.report == 0 and i > 0:
                ...
                if rank == 0:
                    print("{:<10} | {:<7} | {:<13} | {:<15} | {:<15} | {:<15} | {:<15} | {:<12} | {:<6}".format(i,
                         round((i * self.mini_batch - (i // self.full_grad) * self.mini_batch + i // \
                            self.full_grad * (self.data_loader.dataset.data.shape[0] // size)) / \
                         (self.data_loader.dataset.data.shape[0] // size), 2),
                         round(total, 4),
    ...
    def get_full_grad(self, current_weights, loader):
        ...
        for batch_idx, (data, target) in enumerate(loader):
            ...
            grads = [grads[ind] + (len(target) // ((self.data_loader.dataset.data.shape[0]) // size)) *\
                 p.grad.data.detach().clone() for ind, p in enumerate(self.model.parameters())]

        # Return sample gradient
        return grads

    def get_one_stoch_grad(self, current_weights):
        '''Get a local gradient'''
        ...
        for batch_idx, (data, target) in enumerate(self.data_loader):
            ...
            return [p.grad.data.detach().to(self.device) for ind, p in enumerate(self.model.parameters())]
...
if __name__=='__main__':
    ...
    # Parse user input
    parser = argparse.ArgumentParser(description='Testing ProxGT-SR-O/E on problems from paper.')

    parser.add_argument('--updates', type=int, default=10001, help='Total number of communication rounds.')
    parser.add_argument('--lr', type=float, default=5e-3, help='Local learning rate.')
    parser.add_argument('--full_grad', type=int, default=1000, help='When to compute full gradient.')
    ...
    if rank == 0:
        opening_statement = f' ProxGT-SR-O/E on {args.data} '
        print(f"\n{'#' * 75}")
    ...
    # Declare and train!
    algo_params = {'lr': args.lr, 'l1': args.l1,
                   'mini_batch': args.mini_batch, 'full_grad': args.full_grad, 'report': args.report}
    solver = ProxGTSR(algo_params, mixing_matrix, train_loader, init_weights)
